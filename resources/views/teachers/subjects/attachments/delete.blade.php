   <!--primary theme Modal -->
   <div class="modal fade " id="delete_attachment_modal_{{$attachment->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel160" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header bg-danger">
                <h5 class="modal-title white" id="myModalLabel160">
                    {{ Str::limit($attachment->description , 40) }}
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="{{route('teacher.subject.destroy_attachment' , $attachment->id)}}" 
                        method = "post" >
                 @csrf
                 @method('delete')
